...
static gdImagePtr mkwhite(int x, int y) {
	gdImagePtr im;
...
static gdImagePtr mkcross(void) {
	gdImagePtr im;
	int fg, n;
...
static void do_test(void) {
	...
	gdTestAssertMsg(strcasecmp("123abC", "123Abc") == 0,
					"strcasecmp() is not functional.\n");
	...
	for (n = 0; names[n].nm; n++) {
		...
		if (!gdSupportsFileType(names[n].nm, 0)) {
			gdTestAssertMsg(!names[n].required,
							"GD doesn't support required file type: %s\n",
							names[n].nm);
			continue;
		} /* if */
		...
		gdTestAssertMsg(pixels <= names[n].maxdiff,
						"%u pixels different on %s\n", pixels, full_filename);
