...
static void use(gdContextPtr c, gdPaintPtr p)
{
    gdContextSetSource(c, p);
    gdPaintDestroy(p);
}
...
{
    gdImagePtr image = vector2d_create_image(size, size, gdTrueColorAlpha(255, 255, 255, 127));
    gdContextPtr c;
    gdPaintPtr p;
    if (!image)
        return NULL;
    ...
    gdPaintAddColorStopRgb(p, 0, .05, .72, .86);
    gdPaintAddColorStopRgb(p, .5, .17, .16, .48);
    gdPaintAddColorStopRgb(p, 1, .96, .32, .18);
    use(c, p);
    gdContextRectangle(c, 12, 30, size * .68, size * .68);
    gdContextFill(c);
    gdContextSetOperator(c, (gdCompositeOperator)op);
    p = gdPaintCreateRadial(size * .66, size * .40, 2, size * .57, size * .52, size * .48);
    gdPaintAddColorStopRgba(p, 0, 1, .94, .20, .96);
    gdPaintAddColorStopRgba(p, .55, .96, .12, .63, .82);
    gdPaintAddColorStopRgba(p, 1, .20, .02, .55, .12);
    use(c, p);
    gdContextArc(c, size * .58, size * .52, size * .43, 0, 2 * M_PI);
    gdContextFill(c);
    gdContextSetOperator(c, GD_OP_OVER);
    gdContextSetSourceRgba(c, .05, .05, .08, 1);
    label(c, short_names[op], 6, 5, 1.8);
    gdContextDestroy(c);
    return image;
}
...
int main(void)
{
    ...
    for (int op = 0; op < GD_OP_COUNT; op++) {
        gdImagePtr t = render_tile(op, tile);
        int ox = gap + (op % cols) * (tile + gap), oy = gap + (op / cols) * (tile + gap);
        if (!t) {
            gdImageDestroy(out);
            return 1;
        }
    ...
    if (!vector2d_save_png(out, "gradient_operators.png")) {
        gdImageDestroy(out);
        return 1;
    }
    gdImageDestroy(out);
    puts("Saved gradient_operators.png");
    return 0;
}
