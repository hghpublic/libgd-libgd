...
int main(int argc, char **arg)
{
    ...
    new_width = fabs(ceil(cos(a2) * gdImageSX(im))) + fabs(sin(a2) * gdImageSY(im));
    new_height = fabs(ceil(cos(a2) * gdImageSY(im))) + fabs(sin(a2) * gdImageSX(im));
    ...
    gdImageCopyRotated(im2, im, new_width / 2, new_height / 2, 0, 0, gdImageSX(im), gdImageSY(im),
                       angle);
