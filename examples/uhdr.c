...
static int parse_resize(const char *s, gdUhdrImagePtr uhdr, int *out_w, int *out_h)
{
    ...
    if (end != s && *end == 'x') {
        ...
        if (errno != ERANGE && height_end != end + 1 && *height_end == '\0' && w > 0 && h > 0 &&
            w <= INT_MAX && h <= INT_MAX) {
            *out_w = (int)w;
            *out_h = (int)h;
            return 1;
...
static void print_error(const char *where, const gdUhdrError *err)
{
    ...
    fprintf(stderr, "%s failed: code=%d provider=%d message=%s\n", where, err->code,
            err->provider_code, err->message[0] ? err->message : "(none)");
}
...
int main(int argc, char **argv)
{
    ...
    fprintf(stdout, "Loaded: %dx%d gain_map=%d\n", gdUhdrImageWidth(uhdr), gdUhdrImageHeight(uhdr),
            gdUhdrImageHasGainMap(uhdr));
