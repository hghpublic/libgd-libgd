...
int main(void)
{
    ...
    gdImagePtr image = vector2d_create_image(width, height, gdTrueColorAlpha(255, 255, 255, 127));
    ...
    gdContextArc(context, width / 2.0, height / 2.0, width / 3.0, 0, M_PI * 2);
    ...
    for (i = 0; i < sizeof(tiger_commands) / sizeof(tiger_commands[0]); i++) {
        ...
        case 'l':
            ...
            break;
        case 'c':
            gdContextCurveTo(context, cmd->x0, cmd->y0, cmd->x1, cmd->y1, cmd->x2, cmd->y2);
            break;
        case 'f':
