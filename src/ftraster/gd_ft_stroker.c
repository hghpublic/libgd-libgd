...
#define GD_FT_SMALL_CONIC_THRESHOLD (GD_FT_ANGLE_PI / 6)
#define GD_FT_SMALL_CUBIC_THRESHOLD (GD_FT_ANGLE_PI / 8)

#define GD_FT_EPSILON 2
                                           ...
                                           GD_FT_Angle *angle_out)
{
    ...
    close1 = GD_FT_IS_SMALL(d1.x) && GD_FT_IS_SMALL(d1.y);
    close2 = GD_FT_IS_SMALL(d2.x) && GD_FT_IS_SMALL(d2.y);
    ...
    if (close1) {
        if (close2) {
            ...
            *angle_in = *angle_out = GD_FT_Atan2(d2.x, d2.y);
        }
    } else /* !close1 */
    {
        if (close2) {
            *angle_in = *angle_out = GD_FT_Atan2(d1.x, d1.y);
        } else {
            *angle_in = GD_FT_Atan2(d1.x, d1.y);
            *angle_out = GD_FT_Atan2(d2.x, d2.y);
        }
    }

    theta = ft_pos_abs(GD_FT_Angle_Diff(*angle_in, *angle_out));
...
/* have opposite signs.                                    */
static GD_FT_Angle ft_angle_mean(GD_FT_Angle angle1, GD_FT_Angle angle2)
{
                                           ...
                                           GD_FT_Angle *angle_mid, GD_FT_Angle *angle_out)
{
    ...
    close1 = GD_FT_IS_SMALL(d1.x) && GD_FT_IS_SMALL(d1.y);
    close2 = GD_FT_IS_SMALL(d2.x) && GD_FT_IS_SMALL(d2.y);
    close3 = GD_FT_IS_SMALL(d3.x) && GD_FT_IS_SMALL(d3.y);
    ...
    if (close1) {
        if (close2) {
            ...
            {
                *angle_in = *angle_mid = *angle_out = GD_FT_Atan2(d3.x, d3.y);
            }
        } else /* !close2 */
        {
            if (close3) {
                *angle_in = *angle_mid = *angle_out = GD_FT_Atan2(d2.x, d2.y);
            } else /* !close3 */
            {
                *angle_in = *angle_mid = GD_FT_Atan2(d2.x, d2.y);
                *angle_out = GD_FT_Atan2(d3.x, d3.y);
            }
        }
    } else /* !close1 */
    {
        if (close2) {
            if (close3) {
                *angle_in = *angle_mid = *angle_out = GD_FT_Atan2(d1.x, d1.y);
            } else /* !close3 */
            {
                *angle_in = GD_FT_Atan2(d1.x, d1.y);
                *angle_out = GD_FT_Atan2(d3.x, d3.y);
        ...
        } else /* !close2 */
        {
            if (close3) {
                *angle_in = GD_FT_Atan2(d1.x, d1.y);
                *angle_mid = *angle_out = GD_FT_Atan2(d2.x, d2.y);
            } else /* !close3 */
            {
                *angle_in = GD_FT_Atan2(d1.x, d1.y);
                *angle_mid = GD_FT_Atan2(d2.x, d2.y);
                *angle_out = GD_FT_Atan2(d3.x, d3.y);
            }
        }
    }
...
typedef enum GD_FT_StrokeTags_ {
    GD_FT_STROKE_TAG_ON = 1,    /* on-curve point  */
    GD_FT_STROKE_TAG_CUBIC = 2, /* cubic off-point */
    GD_FT_STROKE_TAG_BEGIN = 4, /* sub-path start  */
    GD_FT_STROKE_TAG_END = 8    /* sub-path end    */
...
GD_FT_Error GD_FT_Outline_Check(GD_FT_Outline *outline)
{
    if (outline) {
        ...
        GD_FT_Int n_contours = outline->n_contours;
        GD_FT_Int end0, end;
        GD_FT_Int n;
...
static void ft_stroke_border_close(GD_FT_StrokeBorder border, GD_FT_Bool reverse)
{
    ...
    if (count <= start + 1U)
        ...
        border->tags[start] |= GD_FT_STROKE_TAG_BEGIN;
        border->tags[count - 1] |= GD_FT_STROKE_TAG_END;
    }
                                           ...
                                           GD_FT_Bool movable)
{
    ...
    if (border->movable) {
        ...
        if (border->num_points > 0 &&
            GD_FT_IS_SMALL(border->points[border->num_points - 1].x - to->x) &&
            GD_FT_IS_SMALL(border->points[border->num_points - 1].y - to->y))
            return error;
                                            ...
                                            GD_FT_Vector *control2, GD_FT_Vector *to)
{
    ...
    if (!error) {
        ...
        tag[0] = GD_FT_STROKE_TAG_CUBIC;
        tag[1] = GD_FT_STROKE_TAG_CUBIC;
        tag[2] = GD_FT_STROKE_TAG_ON;
...
static void ft_stroke_border_init(GD_FT_StrokeBorder border)
{
    border->points = NULL;
    border->tags = NULL;
...
static void ft_stroke_border_reset(GD_FT_StrokeBorder border)
{
    border->num_points = 0;
    border->start = -1;
    border->valid = FALSE;
                                               ...
                                               GD_FT_UInt *anum_contours)
{
    ...
    for (; count > 0; count--, num_points++, point++, tags++) {
        ...
        if (tags[0] & GD_FT_STROKE_TAG_END) {
            in_contour = 0;
            num_contours++;
        }
    }
...
static void ft_stroke_border_export(GD_FT_StrokeBorder border, GD_FT_Outline *outline)
{
    ...
    memcpy(outline->points + outline->n_points, border->points,
           border->num_points * sizeof(GD_FT_Vector));
    ...
    /* copy tags */
    {
        ...
        for (; count > 0; count--, read++, write++) {
            if (*read & GD_FT_STROKE_TAG_ON)
                *write = GD_FT_CURVE_TAG_ON;
            else if (*read & GD_FT_STROKE_TAG_CUBIC)
                *write = GD_FT_CURVE_TAG_CUBIC;
            else
                *write = GD_FT_CURVE_TAG_CONIC;
        }
    }
    ...
    assert(GD_FT_Outline_Check(outline) == 0);
}
...
void GD_FT_Stroker_Rewind(GD_FT_Stroker stroker)
{
    if (stroker) {
...
void GD_FT_Stroker_Done(GD_FT_Stroker stroker)
{
    if (stroker) {
...
GD_FT_Error GD_FT_Stroker_LineTo(GD_FT_Stroker stroker, GD_FT_Vector *to)
{
    ...
    angle = GD_FT_Atan2(delta.x, delta.y);
    GD_FT_Vector_From_Polar(&delta, stroker->radius, angle + GD_FT_ANGLE_PI2);
...
GD_FT_Error GD_FT_Stroker_ConicTo(GD_FT_Stroker stroker, GD_FT_Vector *control, GD_FT_Vector *to)
{
    ...
    while (arc >= bez_stack) {
        GD_FT_Angle angle_in, angle_out;
        ...
        } else if (ft_pos_abs(GD_FT_Angle_Diff(stroker->angle_in, angle_in)) >
                   GD_FT_SMALL_CONIC_THRESHOLD / 4) {
            /* if the deviation from one arc to the next is too great, */
            /* add a round corner                                      */
            stroker->center = arc[2];
            stroker->angle_out = angle_in;
            stroker->line_join = GD_FT_STROKER_LINEJOIN_ROUND;
        ...
        /* border                                                          */
        {
            ...
            phi = angle_in + theta;
            length = GD_FT_DivFix(stroker->radius, GD_FT_Cos(theta));
            ...
            for (border = stroker->borders, side = 0; side <= 1; side++, border++) {
                ...
                GD_FT_Vector_From_Polar(&ctrl, length, phi + rotate);
                ctrl.x += arc[1].x;
                ctrl.y += arc[1].y;
                ...
                if (stroker->handle_wide_strokes) {
                    ...
                    alpha1 = GD_FT_Atan2(end.x - start.x, end.y - start.y);
                    ...
                    if (ft_pos_abs(GD_FT_Angle_Diff(alpha0, alpha1)) > GD_FT_ANGLE_PI / 2) {
                        ...
                        blen = GD_FT_Vector_Length(&bvec);

                        sinA = ft_pos_abs(GD_FT_Sin(alpha1 - gamma));
                        sinB = ft_pos_abs(GD_FT_Sin(beta - gamma));

                        alen = GD_FT_MulDiv(blen, sinA, sinB);

                        GD_FT_Vector_From_Polar(&delta, alen, beta);
                        delta.x += start.x;
                        delta.y += start.y;
                                  ...
                                  GD_FT_Vector *control2, GD_FT_Vector *to)
{
    ...
    while (arc >= bez_stack) {
        GD_FT_Angle angle_in, angle_mid, angle_out;
        ...
        /* border                                                          */
        {
            ...
            length1 = GD_FT_DivFix(stroker->radius, GD_FT_Cos(theta1));
            length2 = GD_FT_DivFix(stroker->radius, GD_FT_Cos(theta2));
            ...
            for (border = stroker->borders, side = 0; side <= 1; side++, border++) {
                ...
                GD_FT_Vector_From_Polar(&ctrl1, length1, phi1 + rotate);
                ctrl1.x += arc[2].x;
                ctrl1.y += arc[2].y;

                GD_FT_Vector_From_Polar(&ctrl2, length2, phi2 + rotate);
                ctrl2.x += arc[1].x;
                ctrl2.y += arc[1].y;
...
GD_FT_Error GD_FT_Stroker_BeginSubPath(GD_FT_Stroker stroker, GD_FT_Vector *to, GD_FT_Bool open)
{
    ...
    /* in the `GD_FT_Stroker_EndSubPath' routine.                             */
    /*                                                                     */
...
GD_FT_Error GD_FT_Stroker_EndSubPath(GD_FT_Stroker stroker)
{
    GD_FT_Error error = 0;

    if (stroker->subpath_open) {
        GD_FT_StrokeBorder right = stroker->borders;
...
GD_FT_Error GD_FT_Stroker_ParseOutline(GD_FT_Stroker stroker, const GD_FT_Outline *outline)
{
    ...
    for (n = 0; n < outline->n_contours; n++) {
        GD_FT_UInt last; /* index of last point in contour */
        ...
        while (point < limit) {
            ...
            Do_Conic:
                ...
                error = GD_FT_Stroker_ConicTo(stroker, &v_control, &v_start);
                goto Close;
            ...
            default: /* GD_FT_CURVE_TAG_CUBIC */
            {
                ...
                error = GD_FT_Stroker_CubicTo(stroker, &vec1, &vec2, &v_start);
                goto Close;
            }
            }
        }
