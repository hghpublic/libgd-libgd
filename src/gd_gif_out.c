...
static void GIFEncode(gdIOCtxPtr fp, int GWidth, int GHeight, int GInterlace, int Background,
                      ...
                      gdImagePtr im);
static void GIFAnimEncode(gdIOCtxPtr fp, int IWidth, int IHeight, int LeftOfs, int TopOfs,
                          int GInterlace, int Transparent, int Delay, int Disposal,
                          int BitsPerPixel, int *Red, int *Green, int *Blue, gdImagePtr im);
...
static int _gdImageGifAnimAddCtx(gdImagePtr im, gdIOCtxPtr out, int LocalCM, int LeftOfs,
...
*/
BGD_DECLARE(void) gdImageGifCtx(gdImagePtr im, gdIOCtxPtr out) { _gdImageGifCtx(im, out); }
...
static int _gdImageGifCtx(gdImagePtr im, gdIOCtxPtr out)
{
    ...
    /* All set, let's do it. */
    GIFEncode(out, tim->sx, tim->sy, interlace, 0, tim->transparent, BitsPerPixel, tim->red,
              tim->green, tim->blue, tim);
...
static int comparewithmap(gdImagePtr im1, gdImagePtr im2, int c1, int c2, int *colorMap)
{
    ...
    return (colorMap[c1] = gdImageColorExactAlpha(im2, im1->red[c1], im1->green[c1], im1->blue[c1],
                                                  im1->alpha[c1])) == c2;
}
                                 ...
                                 int TopOfs, int Delay, int Disposal, gdImagePtr previm)
{
    ...
    break_top:
        ...
        if (min_x != 0 || max_x != tim->sx - 1 || min_y != 0 || max_y != tim->sy - 1 ||
            transparent >= 0) {
            ...
            gdImageCopy(pim2, tim, 0, 0, min_x, min_y, max_x - min_x + 1, max_y - min_y + 1);
        ...
        if (transparent >= 0) {
            for (y = 0; y < tim->sy; ++y) {
                for (x = 0; x < tim->sx; ++x) {
                    if (no_changes ||
                        comparewithmap(prev_tim, tim, prev_tim->pixels[min_y + y][min_x + x],
                                       tim->pixels[y][x], 0)) {
    ...
    /* All set, let's do it. */
    GIFAnimEncode(out, tim->sx, tim->sy, LeftOfs, TopOfs, interlace, transparent, Delay, Disposal,
                  BitsPerPixel, LocalCM ? tim->red : 0, tim->green, tim->blue, tim);
    if (pim) {
        /* Destroy palette based temporary image. */
        gdImageDestroy(pim);
    }
...
static const unsigned long masks[] = {0x0000, 0x0001, 0x0003, 0x0007, 0x000F, 0x001F,
