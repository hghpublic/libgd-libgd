...
static inline uint32_t _surface_fetch_pixel_bilinear(gdSurfacePtr image, gd_fixed_t x, gd_fixed_t y,
                                                     gdExtendMode repeat_mode)
{
...
static void operator_argb_color_source_over(uint32_t *dest, int length, uint32_t color,
...
static void operator_argb_color_destination_in(uint32_t *dest, int length, uint32_t color,
                                               uint32_t const_alpha)
{
    gdPremulPixelF src = gdCompositePixelFromArgb32(color);
    for (int i = 0; i < length; i++)
...
static void operator_argb_color_destination_out(uint32_t *dest, int length, uint32_t color,
                                                uint32_t const_alpha)
{
    gdPremulPixelF src = gdCompositePixelFromArgb32(color);
    for (int i = 0; i < length; i++)
...
static void operator_argb_color(gdCompositeOperator op, uint32_t *dest, int length, uint32_t color,
                                uint32_t alpha)
{
    gdPremulPixelF src = gdCompositePixelFromArgb32(color);
    for (int i = 0; i < length; i++)
        dest[i] = gdCompositePixelToArgb32(
            gdCompositePixel(op, src, gdCompositePixelFromArgb32(dest[i]), alpha / 255.0f));
}

#define spanBlendLoop(func)                                                                        \
    while (count--) {                                                                              \
        uint32_t *target =                                                                         \
            (uint32_t *)(surface->data + currentSpan->y * surface->stride) + currentSpan->x;       \
        func(target, currentSpan->len, color, currentSpan->coverage);                              \
        ++currentSpan;                                                                             \
    };

static void argb32_blend_color(gdSurfacePtr surface, gdImageOp op, const gdSpanRlePtr rle,
                               uint32_t color)
{
...
static void argb32_compose_source(uint32_t *dest, int length, const uint32_t *src,
                                  uint32_t const_alpha)
{
    for (int i = 0; i < length; i++)
        dest[i] = gdCompositePixelToArgb32(
            gdCompositePixel(GD_OP_SOURCE, gdCompositePixelFromArgb32(src[i]),
                             gdCompositePixelFromArgb32(dest[i]), const_alpha / 255.0f));
}

static void argb32_compose_source_over(uint32_t *dest, int length, const uint32_t *src,
                                       uint32_t const_alpha)
{
    for (int i = 0; i < length; i++)
        dest[i] = gdCompositePixelToArgb32(
            gdCompositePixel(GD_OP_OVER, gdCompositePixelFromArgb32(src[i]),
                             gdCompositePixelFromArgb32(dest[i]), const_alpha / 255.0f));
}

static void argb32_compose_dst_out(uint32_t *dest, int length, const uint32_t *src,
                                   uint32_t const_alpha)
{
    for (int i = 0; i < length; i++)
        dest[i] = gdCompositePixelToArgb32(
            gdCompositePixel(GD_OP_DEST_OUT, gdCompositePixelFromArgb32(src[i]),
                             gdCompositePixelFromArgb32(dest[i]), const_alpha / 255.0f));
}

static void argb32_compose_dst_in(uint32_t *dest, int length, const uint32_t *src,
                                  uint32_t const_alpha)
{
    for (int i = 0; i < length; i++)
        dest[i] = gdCompositePixelToArgb32(
            gdCompositePixel(GD_OP_DEST_IN, gdCompositePixelFromArgb32(src[i]),
                             gdCompositePixelFromArgb32(dest[i]), const_alpha / 255.0f));
}

static void argb32_compose(gdCompositeOperator op, uint32_t *dest, int length, const uint32_t *src,
                           uint32_t const_alpha)
{
    for (int i = 0; i < length; i++)
        dest[i] = gdCompositePixelToArgb32(gdCompositePixel(op, gdCompositePixelFromArgb32(src[i]),
                                                            gdCompositePixelFromArgb32(dest[i]),
                                                            const_alpha / 255.0f));
}
                                        ...
                                        int fdy, int count, gdSpanPtr spans)
{
    while (count--) {
        ...
        int x = gd_double_to_fixed(pattern->matrix.m01 * cy + pattern->matrix.m00 * cx +
                                   pattern->matrix.m02);
        int y = gd_double_to_fixed(pattern->matrix.m11 * cy + pattern->matrix.m10 * cx +
                                   pattern->matrix.m12);
...
static void argb32_pattern_tiled_blend_transformed(gdSurfacePtr surface, gdImageOp op,
                                                   const gdSpanRlePtr rle,
    ...
    while (count--) {                                                                              \
        ...
        if (sy >= 0 && sy < image_height && sx < image_width) {                                    \
            ...
            if (length > 0) {                                                                      \
                const int coverage = (spans->coverage * pattern->alpha + 127) / 255;               \
                const uint32_t *src =                                                              \
                    (const uint32_t *)(pattern->data + sy * pattern->stride) + sx;                 \
...
static void argb32_pattern_blend_untransformed(gdSurfacePtr surface, gdImageOp op,
                                               const gdSpanRlePtr rle,
                                               const _spans_pattern *pattern)
{
...
void gdDrawBlendPattern(gdContextPtr context, const gdSpanRlePtr rle,
                        const gdPathPatternPtr pattern)
{
    if (pattern == NULL)
        return;
...
static gdPremulPixelF pattern_pixel_at(const _spans_pattern *pattern, int px, int py)
{
    double cx = px + 0.5;
    double cy = py + 0.5;
    gd_fixed_t x = gd_double_to_fixed(pattern->matrix.m01 * cy + pattern->matrix.m00 * cx +
                                      pattern->matrix.m02);
...
static void gradient_sampler_init(gdGradientSampler *s, const gdGradient *g, const gdStatePtr state)
{
    s->gradient = g;
    s->device_to_pattern = g->matrix;
    gdPathMatrixMultiply(&s->device_to_pattern, &s->device_to_pattern, &state->matrix);
...
static gdPremulPixelF gradient_pixel_at(const gdGradientSampler *s, int x, int y)
{
    ...
    return s->valid ? gdGradientSample(s->gradient, &s->device_to_pattern, x + 0.5, y + 0.5) : z;
}
                                ...
                                const gdGradient *gradient)
{
    ...
    while (count--) {
        uint32_t *dst =
            (uint32_t *)(context->surface->data + span->y * context->surface->stride) + span->x;
...
static gdPremulPixelF lerp_clip(gdPremulPixelF dst, gdPremulPixelF result, float coverage)
{
    gdPremulPixelF p;
...
void gdPathBlend(gdContextPtr context, const gdSpanRlePtr rle)
{
    ...
    if (gdCompositeOperatorIsUnbounded(context->state->op) && rle == context->rle) {
