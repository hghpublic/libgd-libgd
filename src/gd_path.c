...
BGD_DECLARE(gdPathPtr) gdPathDuplicate(const gdPathPtr path)
{
    ...
    if ((num_elements && !gdArrayAppendMultiple(&result->elements,
            gdArrayGetData(&path->elements), num_elements)) ||
        (num_points && !gdArrayAppendMultiple(&result->points,
            gdArrayGetData(&path->points), num_points))) {
...
*/
static void _gdPathArc(gdPathPtr path, double cx, double cy, double radius, double angle1, double angle2, int ccw)
{
    if (ccw)
