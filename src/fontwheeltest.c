...
void dowheel(gdImagePtr im, int color, char *fontfile, int fontsize, double angle, int x, int y,
             int offset, char *string)
{
    int brect[8];
    FILE *err;
    ...
    for (curang = 0.0; curang < 360.0; curang += angle) {
        ...
        cp = gdImageStringFT(im, brect, color, fontfile, fontsize, curangrads, x0, y0, string);
        if (cp)
            doerr(err, cp);
...
void dotest(char *font, int size, double incr, int w, int h, char *string, const char *filename)
{
    gdImagePtr im;
    FILE *out;
    int bg;
