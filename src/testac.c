...
void testDrawing(gdImagePtr im_in, double scale, int blending, int palette, char *filename)
{
    ...
    /* Create output image. */
    im_out =
        gdImageCreateTrueColor((int)(gdImageSX(im_in) * scale), (int)(gdImageSY(im_in) * scale));
    /*
    ...
    gdImageFill(im_out, (int)(gdImageSX(im_in) * scale / 2), (int)(gdImageSY(im_in) * scale / 2),
    ...
    gdImageCopyResampled(im_out, im_in, 0, 0, 0, 0, (int)(gdImageSX(im_in) * scale),
                         (int)(gdImageSY(im_in) * scale), gdImageSX(im_in), gdImageSY(im_in));
    /* Write PNG */
