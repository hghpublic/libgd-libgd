...
*/
BGD_DECLARE(void) gdSetErrorMethod(gdErrorMethod error_method) { gd_error_method = error_method; }
 ...
 * http://www.acm.org/jgt/papers/SmithLyons96/hwb_rgb.html, an article on colour
 ...
 * Pure red always maps to 6 in this implementation. Therefore UNDEFINED can be
...
static float HWB_Diff(int r1, int g1, int b1, int r2, int g2, int b2)
{
    ...
    diff =
        diff * diff + (HWB1.W - HWB2.W) * (HWB1.W - HWB2.W) + (HWB1.B - HWB2.B) * (HWB1.B - HWB2.B);
        ...
        for (x = 0; x < (to->sx); x++) {
            ...
            if (xlate[p] == -1) {
                /* This ought to use HWB, but we don't have an alpha-aware
                   version of that yet. */
                xlate[p] = gdImageColorClosestAlpha(from, to->red[p], to->green[p], to->blue[p],
                                                    to->alpha[p]);
                /*printf("Mapping %d (%d, %d, %d, %d) to %d (%d, %d, %d, %d)\n",
                 */
                /*      p,  to->red[p], to->green[p], to->blue[p], to->alpha[p],
                 */
                /*      xlate[p], from->red[xlate[p]], from->green[xlate[p]],
                 * from->blue[xlate[p]], from->alpha[xlate[p]]); */
            };
...
static void gdImageBrushApply(gdImagePtr im, int x, int y)
{
    ...
    if (im->trueColor) {
        if (im->brush->trueColor) {
            ...
            /* 2.0.12: Brush palette, image truecolor (thanks to Thorben Kundinger for pointing out
             * the issue) */
            for (ly = y1; (ly < y2); ly++) {
                srcx = 0;
                for (lx = x1; (lx < x2); lx++) {
                    ...
                    if (im->brush->trueColor) {
                        gdImageSetPixel(im, lx, ly,
                                        gdImageColorResolveAlpha(
                                            im, gdTrueColorGetRed(p), gdTrueColorGetGreen(p),
    ...
    if (im->trueColor) {
        ...
        if (p != gdImageGetTransparent(tile)) {
            if (tile->trueColor) {
                ...
                gdImageSetPixel(
                    im, x, y,
                    gdImageColorResolveAlpha(im, gdTrueColorGetRed(p), gdTrueColorGetGreen(p),
                                             gdTrueColorGetBlue(p), gdTrueColorGetAlpha(p)));
            } else {
...
*/
BGD_DECLARE(void) gdImageAABlend(gdImagePtr im) { (void)im; }
...
static int gdImageTileGet(gdImagePtr im, int x, int y)
{
    ...
    if (p == im->tile->transparent) {
        ...
        if (im->tile->trueColor) {
            ...
            tileColor = gdTrueColorAlpha(gdImageRed(im->tile, p), gdImageGreen(im->tile, p),
                                         gdImageBlue(im->tile, p), gdImageAlpha(im->tile, p));
        }
    } else {
        if (im->tile->trueColor) {
            tileColor = gdImageColorResolveAlpha(im, gdTrueColorGetRed(p), gdTrueColorGetGreen(p),
                                                 gdTrueColorGetBlue(p), gdTrueColorGetAlpha(p));
        } else {
            tileColor =
                gdImageColorResolveAlpha(im, gdImageRed(im->tile, p), gdImageGreen(im->tile, p),
                                         gdImageBlue(im->tile, p), gdImageAlpha(im->tile, p));
        }
    }
    return tileColor;
                ...
                for (x = 0; (x < w); x++) {
                    ...
                    if (c != src->transparent) {
                        gdImageSetPixel(dst, dstX + x, dstY + y,
                                        gdTrueColorAlpha(src->red[c], src->green[c], src->blue[c],
                                                         src->alpha[c]));
                    }
                }
            }
    ...
    for (y = srcY; (y < (srcY + h)); y++) {
        ...
        for (x = srcX; (x < (srcX + w)); x++) {
            ...
            if (src->trueColor) {
                 ...
                 */
                mapTo = gdImageColorResolveAlpha(dst, gdTrueColorGetRed(c), gdTrueColorGetGreen(c),
                                                 gdTrueColorGetBlue(c), gdTrueColorGetAlpha(c));
                ...
                if (dst == src) {
                    ...
                    nc = gdImageColorResolveAlpha(dst, src->red[c], src->green[c], src->blue[c],
                                                  src->alpha[c]);
                }
                colorMap[c] = nc;
                mapTo = colorMap[c];
    ...
    for (y = srcY; (y < (srcY + h)); y++) {
        ...
        for (x = srcX; (x < (srcX + w)); x++) {
            ...
            if (dst == src && pct == 100) {
                ...
                dc = gdImageGetPixel(dst, tox, toy);
                g = 0.29900 * gdImageRed(dst, dc) + 0.58700 * gdImageGreen(dst, dc) +
                    0.11400 * gdImageBlue(dst, dc);

                ncR = gdImageRed(src, c) * (pct / 100.0) + g * ((100 - pct) / 100.0);
                ncG = gdImageGreen(src, c) * (pct / 100.0) + g * ((100 - pct) / 100.0);
                ncB = gdImageBlue(src, c) * (pct / 100.0) + g * ((100 - pct) / 100.0);
        ...
        for (ydest = 0; (ydest < sty[y - srcY]); ydest++) {
            ...
            for (x = srcX; (x < (srcX + srcW)); x++) {
                ...
                if (dst->trueColor) {
                    ...
                    if (src->trueColor) {
                        /* Remap to the palette available in the destination image. This is slow and
                         * works badly. */
                        mapTo = gdImageColorResolveAlpha(
                            dst, gdTrueColorGetRed(c), gdTrueColorGetGreen(c),
                            gdTrueColorGetBlue(c), gdTrueColorGetAlpha(c));
                    } else {
                        /* Have we established a mapping for this color? */
                        if (colorMap[c] == (-1)) {
                            ...
                            if (dst == src) {
                                ...
                                nc = gdImageColorResolveAlpha(
                                    dst, gdImageRed(src, c), gdImageGreen(src, c),
                                    gdImageBlue(src, c), gdImageAlpha(src, c));
                            }
                            colorMap[c] = nc;
                        }
                        mapTo = colorMap[c];
                   ...
                   int srcWidth, int srcHeight, int angle)
{
    ...
    */
    ...
    if (dxMin < safe_min + radius + scX)
        dxMin = safe_min + radius + scX;
    if (dxMax > safe_max - radius - scX)
        dxMax = safe_max - radius - scX;
    if (dyMin < safe_min + radius + scY)
        dyMin = safe_min + radius + scY;
    if (dyMax > safe_max - radius - scY)
        dyMax = safe_max - radius - scY;
    ...
    for (dy = dyMin; dy <= dyMax; dy++) {
        for (dx = dxMin; dx <= dxMax; dx++) {
            ...
            int sx = sxd + scX;
            int sy = syd + scY;
            if ((sx >= srcX) && (sx < srcX + srcWidth) && (sy >= srcY) && (sy < srcY + srcHeight)) {
                ...
                if (c == src->transparent) {
                    ...
                    if (cmap[c] == -1) {
                        cmap[c] =
                            gdImageColorResolveAlpha(dst, gdImageRed(src, c), gdImageGreen(src, c),
                                                     gdImageBlue(src, c), gdImageAlpha(src, c));
                    }
    ...
    for (y = dstY; (y < dstY + dstH); y++) {
        for (x = dstX; (x < dstX + dstW); x++) {
            ...
            gdImageSetPixel(dst, x, y,
                            gdTrueColorAlpha((int)red, (int)green, (int)blue, (int)alpha));
        }
    }
}
...
BGD_DECLARE(void) gdImageFilledPolygon(gdImagePtr im, gdPointPtr p, int n, int c)
{
    ...
    for (y = miny; (y <= maxy); y++) {
        ...
        for (i = 0; (i < n); i++) {
            ...
            if ((y >= y1) && (y < y2)) {
                im->polyInts[ints++] =
                    (int)((float)((y - y1) * (x2 - x1)) / (float)(y2 - y1) + 0.5 + x1);
...
BGD_DECLARE(void) gdImageSetBrush(gdImagePtr im, gdImagePtr brush)
{
    ...
    if ((!im->trueColor) && (!im->brush->trueColor)) {
        for (i = 0; (i < gdImageColorsTotal(brush)); i++) {
            int index;
            index = gdImageColorResolveAlpha(im, gdImageRed(brush, i), gdImageGreen(brush, i),
...
BGD_DECLARE(void) gdImageSetTile(gdImagePtr im, gdImagePtr tile)
{
    ...
    if ((!im->trueColor) && (!im->tile->trueColor)) {
        for (i = 0; (i < gdImageColorsTotal(tile)); i++) {
            int index;
            index = gdImageColorResolveAlpha(im, gdImageRed(tile, i), gdImageGreen(tile, i),
...
{
#if ENABLE_CORRECTED_LEGACY_COMPOSITING
    return gdCompositePixelToGd(gdCompositePixel(GD_OP_OVER, gdCompositePixelFromGd(src),
    ...
    green = (gdTrueColorGetGreen(src) * src_weight + gdTrueColorGetGreen(dst) * dst_weight) /
            tot_weight;
    blue =
        (gdTrueColorGetBlue(src) * src_weight + gdTrueColorGetBlue(dst) * dst_weight) / tot_weight;
...
{
#if ENABLE_CORRECTED_LEGACY_COMPOSITING
    return gdCompositePixelToGd(gdCompositePixel(GD_OP_OVERLAY, gdCompositePixelFromGd(src),
                                                 gdCompositePixelFromGd(dst), 1.0f));
#else
    int a1, a2;
    ...
    return (
        ((gdAlphaMax - a1 * a2 / gdAlphaMax) << 24) +
...
{
#if ENABLE_CORRECTED_LEGACY_COMPOSITING
    return gdCompositePixelToGd(gdCompositePixel(GD_OP_MULTIPLY, gdCompositePixelFromGd(src),
                                                 gdCompositePixelFromGd(dst), 1.0f));
#else
